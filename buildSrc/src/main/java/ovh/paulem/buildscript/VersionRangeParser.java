...
public class VersionRangeParser {
    ...
    public static Iterable<? extends String> parseVersionRange(String min_version_range, String max_version_range) {
        List<String> allVersions = getAllMinecraftVersions().reversed();
        int startElement = allVersions.indexOf(min_version_range);
        int endElement = allVersions.indexOf(max_version_range);
        ...
        try {
            ...
            try (InputStreamReader reader = new InputStreamReader(connection.getInputStream())) {
                ...
                for (int i = 0; i < versionsArray.size(); i++) {
                    ...
                    if (versionId.matches("1\\.\\d+(\\.\\d+)?")) { // Matches 1.x or 1.x.x
